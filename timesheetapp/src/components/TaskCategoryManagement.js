...
class TaskCategoryManagement extends Component {
  ...
  handleAddOrUpdate = async (e) => {
    ...
    try {
      ...
      const url = editingId
        ? `http://localhost:8080/api/task-categories/${editingId}`
        : 'http://localhost:8080/api/task-categories/post_taskCategory';
  ...
  handleDelete = async (categoryId) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      ...
      try {
        const accessToken = localStorage.getItem('accessToken') || 'mock-token';
        const response = await fetch(`http://localhost:8080/api/task-categories/${categoryId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          },
